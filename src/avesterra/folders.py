...
def create_folder(
    ...
    >>> import avesterra.folders as folders  # Create a basic folder
...
def delete_folder(
    ...
    >>> import avesterra.folders as folders  # Delete a folder
...
def insert_folder(
    ...
    >>> import avesterra.folders as folders  # Insert a property
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
...
def remove_folder(
    ...
    >>> import avesterra.folders as folders  # Remove property at index 1
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> folders.remove_folder(folder=folder, index=1, authorization=authorization)

    >>> import avesterra.folders as folders  # Remove with parameters
...
def replace_folder(
    ...
    >>> import avesterra.folders as folders  # Replace property at index 2
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
...
def find_item(
    ...
    >>> import avesterra.folders as folders  # Find item by value
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
...
def lookup_item(
    ...
    >>> import avesterra.folders as folders  # Lookup item by key
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    ...
    >>> import avesterra.folders as folders  # Lookup with error handling
    >>> try:
...
def item_count(folder: AvFolder, authorization: AvAuthorization = NULL_AUTHORIZATION):
    ...
    >>> import avesterra.folders as folders  # Get property count
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> count = folders.item_count(folder=folder, authorization=authorization)
    >>> print(f"Folder contains {count} properties")

    >>> import avesterra.folders as folders  # Check if folder is empty
...
def item_member(
    ...
    >>> import avesterra.folders as folders  # Check if key exists
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    ...
    >>> import avesterra.folders as folders  # Validate before access
    >>> key = "important_file"
...
def item_name(
    ...
    >>> import avesterra.folders as folders  # Get name at index 1
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> name = folders.item_name(folder=folder, index=1, authorization=authorization)
    >>> print(f"Property name: {name}")

    >>> import avesterra.folders as folders  # Iterate through property names
...
def item_key(
    ...
    >>> import avesterra.folders as folders  # Get key at index 2
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> key = folders.item_key(folder=folder, index=2, authorization=authorization)
    >>> print(f"Property key: {key}")

    >>> import avesterra.folders as folders  # Get all keys
    >>> count = folders.item_count(folder, authorization)
...
def item_value(
    ...
    >>> import avesterra.folders as folders  # Get value at index 1
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> value = folders.item_value(folder=folder, index=1, authorization=authorization)
    >>> print(f"Property value: {value}")

    >>> import avesterra.folders as folders  # Get all values
    >>> count = folders.item_count(folder, authorization)
...
def save_folder(folder: AvFolder, authorization: AvAuthorization = NULL_AUTHORIZATION):
    ...
    >>> import avesterra.folders as folders  # Save folder changes
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> folders.save_folder(folder=folder, authorization=authorization)
    >>> print("Folder saved successfully")

    >>> import avesterra.folders as folders  # Save after batch operations
...
def erase_registry(
    ...
    >>> import avesterra.folders as folders  # Erase all properties
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> folders.erase_registry(folder=folder, authorization=authorization)
    >>> print("All properties erased from folder")

    >>> import avesterra.folders as folders  # Clear and verify
...
def sort_registry(
    ...
    >>> import avesterra.folders as folders  # Sort folder properties
    >>> folder: AvFolder
    >>> authorization: AvAuthorization
    >>> folders.sort_registry(folder=folder, authorization=authorization)
    >>> print("Folder properties sorted")

    >>> import avesterra.folders as folders  # Sort and save
